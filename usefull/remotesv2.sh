...
display_info() {
  ...
  read -p "Enter the number of the connection to stop, 'p' to ping a remote connection, or 'c' to start a chat session: " input
  ...
  if [[ $input =~ ^[0-9]+$ ]]; then
    ...
    # Prompt the user for the remote IP and port
    read -p "Enter the remote IP: " remote_ip
    read -p "Enter the remote port: " remote_port
    
    # Start a netcat chat session with the remote host
